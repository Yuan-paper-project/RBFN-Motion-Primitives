...
from ml_planner.planner.networks.layers.basis_functions import basis_func_dict
...
class ModelTrainer:
    def __init__(self, train_dataloader: DataLoader,
        ...
        self.input_idxs = [train_dataloader.dataset.dataset.input_labels.index(i) for i in model_kwargs['input_labels']]
        self.output_idxs = [train_dataloader.dataset.dataset.labels.index(i) for i in model_kwargs['output_labels']]
        print(self.model)

        # Create and save config of model
        path = f"{self.model_path}/config.json"
        try:
            model_kwargs["bounds"] = model_kwargs["bounds"].tolist()
        except KeyError:
            pass
        model_kwargs['basis_func'] = model_kwargs['basis_func'].__name__
        ...
        self.reduction = "mean"
        self.loss_fns = [loss_fn(labels=self.model.output_labels, reduction=self.reduction)
    ...
    def _validate_input(self, model_kwargs, train_dataloader):
        ...
        assert model_kwargs['basis_func'] in basis_funcs, \
        ...
        assert all([i in labels for i in model_kwargs['input_labels']]), \
            f"Unknown input labels <{model_kwargs['input_labels']}>, only <{labels}> are supported"
        labels = train_dataloader.dataset.dataset.labels
        assert all([i in labels for i in model_kwargs['output_labels']]), \
            f"Unknown output labels <{model_kwargs['output_labels']}>, only <{labels}> are supported"

    def train(self, epoch: int):
        """Training routine for the model"""
        self.model.train()
        running_losses = [0.]*(len(self.loss_fns) + 1)
        total_steps = len(self.train_dataloader)-1

        pbar = tqdm.tqdm(enumerate(self.train_dataloader), total=total_steps, position=1,
                         leave=False, desc="Sample", ncols=100, disable=False)

        for idx, (input_vec, gt_data) in pbar:
            # get data of used labels
            input_vec = input_vec[:, self.input_idxs].to(self.device)
            gt_vec = gt_data[:, self.output_idxs, :].to(self.device)
            ...
            if idx == 0:
                for tag, value in self.model.named_parameters():
                    if value.grad is not None:
                        self.writer.add_histogram(f"Grad/{tag}", value.grad, epoch + 1)
                        self.writer.add_scalar(f"Grad_scalar/{tag}", value.grad.norm().item(), epoch + 1)
            ...
            if ((epoch + 1) % self.draw_trajectories == 0 or epoch == 0) and idx in [0, total_steps]:
                self.writer.add_figure(f"train idx {idx}",
                                       self.draw_trained_trajectories(pred, gt_vec), epoch + 1)
        ...
        try:
            self.writer.add_scalar("Param/RBF Center", self.model.rbflayer.centers.norm().item(), current)
            self.writer.add_scalar("Param/RBF Eps", self.model.rbflayer.eps.norm().item(), current)
            self.writer.add_scalar("Param/Dist", self.model.rbflayer.distance.norm().item(), current)
            self.writer.add_scalar("Param/Output", self.model.rbflayer.output.norm().item(), current)
            self.writer.add_histogram("RBF/Center", self.model.rbflayer.centers, current)
            self.writer.add_histogram("RBF/Eps", self.model.rbflayer.eps, current)
            self.writer.add_histogram("RBF/Dist", self.model.rbflayer.distance, current)
            self.writer.add_histogram("RBF/Output", self.model.rbflayer.output, current)

            self.writer.add_scalar("Param/Linear Weigths", self.model.linear.weight.norm().item(), current)
            self.writer.add_histogram("Linear/Weigths", self.model.linear.weight, current)
        except AttributeError:
            pass
        try:
            self.writer.add_histogram("Linear/Bias", self.model.linear.bias, current)
            self.writer.add_scalar("Param/Linear Bias", self.model.linear.bias.norm().item(), current)
        except AttributeError:
            pass
        try:
            self.writer.add_scalar("Param/Input Linear Weigths", self.model.input_layer.weight.norm().item(), current)
            self.writer.add_scalar("Param/Input Linear Bias", self.model.input_layer.bias.norm().item(), current)
        except AttributeError:
            pass
        try:
            self.writer.add_histogram("Input Linear/Weigths", self.model.input_layer.weight, current)
            self.writer.add_histogram("Input Linear/Bias", self.model.input_layer.bias, current)
        except AttributeError:
            pass
        self.writer.flush()
        return avg_losses[0]
        ...
        with torch.no_grad():
            ...
            for idx, (input_vec, gt_data) in enumerate(self.test_dataloader):
                ...
                if ((epoch + 1) % self.draw_trajectories == 0 or epoch == 0) and idx in [0, last_idx]:
                    self.writer.add_figure(f"test idx {idx}",
                                           self.draw_trained_trajectories(pred, gt_vec), epoch + 1)
    ...
    def run_training(self, epochs=30):
        ...
        pbar = tqdm.tqdm(range(epochs), position=0, leave=False, desc="Epochs", ncols=100,
    ...
    def draw_trained_trajectories(self, pred_trajs, gt_trajs):
        ...
        if pos:
            ...
            ax.plot(pred_trajs[0, idx_x, :], pred_trajs[0, idx_y, :], color='r', label='Prediction')

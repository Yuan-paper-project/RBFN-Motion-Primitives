...
class RBFLayernD(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, basis_func: Callable):
        ...
        self.centers = nn.Parameter(torch.empty(self.in_features, self.out_features, dtype=torch.float32))
        self.eps = nn.Parameter(torch.empty(out_features, dtype=torch.float32))

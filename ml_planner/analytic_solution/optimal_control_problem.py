...
It is extended from the :mod:`control.optimal` module of the python control system library
...
from control.iosys import _process_indices, _process_labels, \
...
class OptimalControlProblem():
    ...
    def __init__(
            ...
            terminal_cost=None,
            cost_derivative=None, terminal_cost_derivative=None,
            use_constraint_derivative=False,
        ...
        self.d_integral_cost = cost_derivative
        self.d_terminal_cost = terminal_cost_derivative
        self.use_constraint_derivative = use_constraint_derivative
        ...
        if trajectory_method is None:
            ...
            raise NotImplementedError(f"Unkown method {trajectory_method}")
        ...
        if len(self.constraint_lb) != 0:
            jac = self._constraint_function_jac if self.use_constraint_derivative else "2-point"
            self.constraints.append(sp.optimize.NonlinearConstraint(
                self._constraint_function, self.constraint_lb,
                self.constraint_ub, jac=jac))

        if len(self.eqconst_value) != 0:
            jac = self._eqconst_function_jac if self.use_constraint_derivative else "2-point"
            self.constraints.append(sp.optimize.NonlinearConstraint(
                self._eqconst_function, self.eqconst_value,
                self.eqconst_value, jac=jac))

        if self.collocation:
            # Add the collocation constraints
            jac = self._collocation_constraint_jac if self.use_constraint_derivative else "2-point"
            colloc_zeros = np.zeros(sys.nstates * self.timepts.size)
            self.colloc_vals = np.zeros((sys.nstates, self.timepts.size))
            self.constraints.append(sp.optimize.NonlinearConstraint(
                self._collocation_constraint, colloc_zeros, colloc_zeros, jac=jac))
    ...
    def _cost_function_jac(self, coeffs):
        if self.log:
            start_time = time.process_time()
            logging.info("_cost_function called at: %g", start_time)
        ...
        if ct.isctime(self.system):
            # Evaluate the costs
            cost_jac = [self.d_integral_cost(states[:, i], inputs[:, i]) for i in range(self.timepts.size)]

            # Integrate the cost
            cost_jac = np.array(cost_jac)

        else:
            # Sum the integral cost over the time (second) indices
            cost_jac = sum(map(
                self.d_integral_cost, states[:, :-1].transpose(),
                inputs[:, :-1].transpose()))

        # Terminal cost
        if self.d_terminal_cost is not None:
            cost_jac[-1, :] += self.d_terminal_cost(states[:, -1], inputs[:, -1])

        # Return the total cost for this input sequence
        cost_jac = np.hstack([cost_jac[:, self.system.nstates:].T.reshape(-1),
                              cost_jac[:, :self.system.nstates].reshape(-1)])
        # Update statistics
        if self.log:
            stop_time = time.process_time()
            self.cost_process_time += stop_time - start_time
            logging.info(
                "_cost_function_jac returning after elapsed time: %g",
                # cost_jac,
                stop_time - start_time)

        return cost_jac.flatten()
    ...
    def _constraint_function(self, coeffs):
        ...
        for i, t in enumerate(self.timepts):
            for ctype, fun, lb, ub in self.trajectory_constraints:
                if np.all(lb == ub):
                    ...
                    ncol = self.timepts.size - 1
                    zeros = np.zeros((fun.shape[0], ncol))
                    jac_loc = np.hstack([np.column_stack((fun[:, j], zeros))
                                         for j in range(fun.shape[1])])
                    jac.append(np.hstack([jac_loc[:, -self.system.ninputs*self.timepts.size-i:],
                                          jac_loc[:, :-self.system.ninputs*self.timepts.size-i]]))
        ...
        for ctype, fun, lb, ub in self.terminal_constraints:
            if np.all(lb == ub):
                ...
                ncol = self.timepts.size - 1
                zeros = np.zeros((fun.shape[0], ncol))
                jac_loc = np.hstack([np.column_stack((zeros, fun[:, i]))
                                     for i in range(fun.shape[1])])
                jac.append(np.hstack([jac_loc[:, -self.system.ninputs*self.timepts.size:],
                                      jac_loc[:, :-self.system.ninputs*self.timepts.size]]))
            elif ctype == opt.NonlinearConstraint:
        ...
        self.jac_constraint = np.vstack(jac)
        return np.hstack(value)

    def _constraint_function_jac(self, coeffs):
        assert all([ctype == opt.LinearConstraint for ctype, _, _, _ in self.trajectory_constraints]), \
             "Only linear constraints are supported for jacobian computation"

        # jac  = np.eye(len(self.constraint_lb))
        return self.jac_constraint
    ...
    def _eqconst_function(self, coeffs):
        ...
        # Return the value of the constraint function
        self.jac_eqconst = np.vstack(jac)
        return np.hstack(value)

    def _eqconst_function_jac(self, coeffs):
        return self.jac_eqconst
    ...
    def _collocation_constraint(self, coeffs):
        ...
        if self.system.isctime():
            ...
            jac = np.zeros((self.system.nstates * self.timepts.size, len(coeffs)))
            ...
            for i, t in enumerate(self.timepts):
                if i == 0:
                    ...
                    dfk = self.system.d_rhs(self.timepts[0], states[:, 0], inputs[:, 0], self.system.params)
                    # for j in range(self.system.nstates):
                    #     jac[0,0] = 1.
                    continue
                ...
                dfk1 = self.system.d_rhs(t, states[:, i], inputs[:, i], self.system.params)

                for j in range(self.system.nstates):
                    jac[i-1 + self.timepts.size * j, i-1 + self.timepts.size * j] = 1.
                    for k in range(self.system.nstates+self.system.ninputs):
                        jac[i + self.timepts.size * j, i + self.timepts.size * j-1:i + self.timepts.size * j+1] = \
                            [-1, 1]
                        jac[i + self.timepts.size * j, i+self.timepts.size * k] = \
                            -0.5 * (self.timepts[i] - self.timepts[i-1]) * dfk1[j, k]
                        jac[i + self.timepts.size * j, i+self.timepts.size * k-1] = \
                            -0.5 * (self.timepts[i] - self.timepts[i-1]) * dfk[j, k]

                dfk = dfk1

            self.jac_collocation = np.hstack((jac[:, -self.timepts.size*self.system.ninputs:],
                                              jac[:, :-self.timepts.size*self.system.ninputs]))
    ...
    def _collocation_constraint_jac(self, coeffs):
        # raise ValueError("Jacobian not implemented correctly for collocation constraints")
        return self.jac_collocation
    ...
    def compute_trajectory(
        ...
        jac = self._cost_function_jac if (self.d_integral_cost and self.d_terminal_cost) is not None else None
...
def solve_ocp(
    ...
    ocp = OptimalControlProblem(
        ...
        cost_derivative=cost_derivative, terminal_cost_derivative=terminal_cost_derivative,
        use_constraint_derivative=use_constraint_derivative,
...
def quadratic_cost(sys, Q, R, x0=0, u0=0):
    ...
    dcost_fun : callable
        Function that can be used to evaluate the gradient of the cost at a
        given state and input.  The call signature of the function is
        dcost_fun(x, u).
    """
    ...
    if Q is None:
        return lambda x, u: ((u-u0) @ R @ (u-u0)).item(), lambda x, u: 2 * (R @ (u-u0))

    if R is None:
        return lambda x, u: ((x-x0) @ Q @ (x-x0)).item(), lambda x, u: 2 * Q @ (x-x0)
    # Received both Q and R matrices
    return lambda x, u: ((x-x0) @ Q @ (x-x0) + (u-u0) @ R @ (u-u0)).item(), \
        lambda x, u: 2 * (np.hstack([Q @ (x-x0), R @ (u-u0)]))
